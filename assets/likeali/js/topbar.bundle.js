...
var utils = function () {
  ...
  (0, _createClass3.default)(utils, null, [{
    ...
    key: "deleteCookies",
    value: function deleteCookies(name, domain) {
      var date = new Date();
    ...
    key: "transArrayToMapByKey",
    value: function transArrayToMapByKey(source, key) {
      var result = {};
      key = key ? key : "id";
      if (source) {
    ...
    key: "sliceArray",
    value: function sliceArray(arraySource, colLength) {
      var result = [];
    ...
    key: "sliceArrayByColumn",
    value: function sliceArrayByColumn(arrTarget, columns, type) {
      var result = [];
      var layoutConfig = {
        "char_4_23": [7, 9, 5, 2],
        "char_4_24": [7, 10, 5, 2],
    ...
    key: "existKeyInMap",
    value: function existKeyInMap(keyList, keyMap) {
      var newList = [];
    ...
    key: "setCookiesNavigationVersion",
    value: function setCookiesNavigationVersion(version) {
      this.setCookie("consoleNavVersion", version, ".console.aliyun.com");
    }
  }, {
    key: "getCookiesNavigationVersion",
    value: function getCookiesNavigationVersion() {
      return this.getCookie("consoleNavVersion");
    }
  }, {
    key: "loadScript",
    value: function loadScript(scripts) {
      if (!_jquery2.default) return;

      if (!_jquery2.default.isArray(scripts)) {
        scripts = [scripts];
      }

      scripts.forEach(function (script) {
        _jquery2.default.ajax(script);
      });
    }
    ...
    key: "getColumnCountByScreen",
    value: function getColumnCountByScreen() {
    ...
    key: "getUrlParamByKey",
    value: function getUrlParamByKey(key, url) {
    ...
    key: "getGroupPosInfo",
    value: function getGroupPosInfo(index, groupList) {
    ...
    key: "loadExtraScripts",
    value: function loadExtraScripts(extraScripts) {
      var url = '';
      var el = 'body';
      ...
      _jquery2.default.each(extraScripts, function (key, item) {
        var el = item.appendTo || 'body';

        if (item.type === 'css') {
          (0, _jquery2.default)(el).append("<link type=\"text/css\" rel=\"stylesheet\" href=\"" + item.url + "\" />");
        } else {
          (0, _jquery2.default)(el).append("<script src=\"" + item.url + "\"></script>");
        }
      });
    ...
    key: "combineExtraScripts",
    value: function combineExtraScripts(extraScripts) {
    ...
    key: "combineExtraFile",
    value: function combineExtraFile(extraFiles) {
      var result = [];
    ...
    value: function getLang() {
      return utils.getCookie('aliyun_lang') || 'zh';
    }
  }, {
    key: "getEnv",
    ...
    value: function getAPIUrl(path) {
      ...
      var APIDomainConfig = {
        dev: '//mocks.alibaba-inc.com/mock/maqing-mocks',
        test: '//fecs.console.aliyun.test',
        prod: '//fecs.console.aliyun.com'
      };

      var APIDomain = APIDomainConfig[env];
    ...
    value: function getVersionUrl() {
      return this.getAPIUrl('api/topbar/version');
    }
    ...
    key: "getGrayUrl",
    value: function getGrayUrl() {
      return this.getAPIUrl('api/topbar/gray');
    }
  }, {
    key: "indexOf",
    value: function indexOf(arr, val) {
      if (Object.prototype.toString.call(arr) != '[object Array]') return -1;
      var index = -1;
    ...
    value: function contains(arr, val) {
      if (this.indexOf(arr, val) !== -1) return true;
    ...
    key: "loadScript",
    value: function loadScript(url, callback, errCallback) {
...
utils.ls = {
  ...
  /**
   * 从缓存中读取对应 key 的值，如果读不到用 defaultVal, 如果设置了 fn，则调用 fn，并返回结果
   */
  getItem: function getItem(key, defaultVal, fn) {
    if (localStorage) {
      return (typeof fn === 'function' ? fn(localStorage.getItem(key)) : localStorage.getItem(key)) || defaultVal;
    }
...
var toObject = __webpack_require__(29);
var toLength = __webpack_require__(15);
var asc = __webpack_require__(296);
...
$export($export.S, 'Math', { fround: __webpack_require__(302) });
...
if (__webpack_require__(17) && /./g.flags != 'g') __webpack_require__(11).f(RegExp.prototype, 'flags', {
  configurable: true,
...
var _topbar = __webpack_require__(177);
...
var ConsoleTopbar = function () {
  function ConsoleTopbar($injector) {
    ...
    (0, _classCallCheck3.default)(this, ConsoleTopbar);
  ...
  (0, _createClass3.default)(ConsoleTopbar, [{
    key: 'render',
    value: function () {
      var _ref = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee(scope, element, attr, responseData) {
        var aliyun_site, ls, left, viewFrameworkConfig, defaultVersion, console_version;
        return _regenerator2.default.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                //如果是日本站
                aliyun_site = _utils2.default.getCookie('aliyun_site');
                ls = _utils2.default.ls;

                // 使用 v3 版本的 topbar 风格

                this.viewFrameworkSetting.setVersion('v3');

                // 新增加一个容器，用来渲染topbar & sidebar
                $('body').prepend('<div class="viewframeContainer"></div>').css({
                  'background-color': '#373d41'
                }).addClass('newTopbar');

                // 移除老的topbar & sidebar
                $('.viewFramework-topbar').remove();
                $('.viewFramework-sidebar').remove();

                // 样式优化
                $('.viewFramework-sidebar-mini .viewFramework-product').css('left', '0');
                $('.viewFramework-sidebar-full .viewFramework-product').css('left', '0');
                left = $('body').hasClass('sidebar-full') ? '180px' : '50px';
                viewFrameworkConfig = this.viewFrameworkSetting.config;

                // 隐藏了sidebar的场景

                if (viewFrameworkConfig.hideSidebar || _utils2.default.getUrlParamByKey('hideSidebar')) {
                  left = 0;
                }

                $('.viewFramework-body').css({
                  left: left,
                  right: 0,
                  width: 'auto'
                });

                // 映射 viewFrameworkSetting 到全局变量 viewframeSetting 中
                window.viewframeSetting = $.extend(window.viewframeSetting || {}, {
                  productId: this.productId,
                  selector: '.viewframeContainer',
                  regionList: viewFrameworkConfig.productRegionList || [],
                  defaultRegionId: viewFrameworkConfig.defaultRegionId || 'cn-hangzhou',
                  showRegionbar: viewFrameworkConfig.showRegionbar || false,
                  isGlobal: viewFrameworkConfig.isGlobal || false,
                  regionIdKey: viewFrameworkConfig.regionIdKey || 'regionId',
                  hideSidebar: viewFrameworkConfig.hideSidebar || false,
                  hideTopbar: viewFrameworkConfig.hideTopbar || false,
                  pathRules: viewFrameworkConfig.pathRules || [],
                  regionSetting: viewFrameworkConfig.regionSetting || {},
                  recentEnabled: typeof viewFrameworkConfig.recentEnabled != 'undefined' ? viewFrameworkConfig.recentEnabled : true,
                  isRedirect: typeof viewFrameworkConfig.isRedirect != 'undefined' ? viewFrameworkConfig.isRedirect : true,
                  onRegionChange: viewFrameworkConfig.onRegionChange || null
                });

                defaultVersion = '0.0.44';
                console_version = defaultVersion;
                _context.prev = 15;
                _context.next = 18;
                return _utils2.default.getVersion();

              case 18:
                console_version = _context.sent;


                if (console_version && console_version.successResponse) {
                  // 获取公共库版本号，默认降级到0.0.43
                  console_version = console_version.data && console_version.data.baseVersion;

                  // 缓存版本号信息
                  ls.setItem('baseVersion', console_version);
                } else {
                  // 先判断是否存在缓存版本，有缓存版本，优先使用，否则使用兜底版本
                  console_version = ls.getItem('baseVersion', defaultVersion);
                }
                _context.next = 25;
                break;
              ...
              case 25:
                ...
                // 注入 topbar & sidebar 脚本
                $('body').append('<script src="//g.alicdn.com/aliyun/console-base/' + console_version + '/app.js"></script>');
                ...
                window.addEventListener('message', function (event) {
                  ...
                  // 切换sidebar 状态
                  if (type === 'TOGGLE_SIDEBAR' || type === 'SIDEBAR_TYPE') {
                    $('.viewFramework-body').css({
                      left: payload.isSidebarFold ? '50px' : '180px'
                    });
                  }
                });
